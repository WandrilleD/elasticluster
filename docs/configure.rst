...
``vm_deployment_template``
   Path to a JSON file containing the Azure Resource Manager template
   used for starting a new VM.  The `default ARM template`_ used by
   ElastiCluster uses the `API profile *2018-03-01-hybrid*`_; however,
   newer features may be available in some zones which are not part of
   the global API profile.  In order to use specific features or more
   modern service API versions, you can instruct ElastiCluster to use
   an alternate ARM template with this option.

   .. note::

      Use of this option requires that you know how to write or modify
      an ARM template.  It may be better to first ask on the
      ElastiCluster `mailing-list`_ before starting such an endeavour.
   ...
   The following parameters are available to the ARM template at
   instanciation time:
   ...
   .. list-table:: ARM template parameters
      ...
      * - ``imagePublisher``
        - Publisher of the disk image to use (e.g., ``Canonical``).
          See `<https://docs.microsoft.com/en-us/cli/azure/vm/image?view=azure-cli-latest#commands>`_
          for instructions on how to list information about available images.
      * - ``imageOffer``
        - Category of the disk image to use (e.g., ``UbuntuServer``).
          See `<https://docs.microsoft.com/en-us/cli/azure/vm/image?view=azure-cli-latest#commands>`_
          for instructions on how to list information about available images.
      * - ``imageSku``
        - SKU of the disk image to use (e.g., ``16.04.0-LTS``).
          See `<https://docs.microsoft.com/en-us/cli/azure/vm/image?view=azure-cli-latest#commands>`_
          for instructions on how to list information about available images.
      * - ``imageVersion``
        - Version of the disk image to use (e.g., ``latest``).
          See `<https://docs.microsoft.com/en-us/cli/azure/vm/image?view=azure-cli-latest#commands>`_
          for instructions on how to list information about available images.
      * - ``networkSecurityGroupName``
        - name of *security group* to place VMs of the cluster into;
          gotten from configuration option ``security_group``.
      * - ``sshKeyData``
        - content of SSH public key to deploy on VMs
      * - ``storageAccountName``
        - Unique string identifying the storage account for the disks
          attached to VMs in a cluster.  Automatically generated by
          ElastiCluster.
      * - ``storageAccountType``
        - Type of disks to attach to the VM. See option
          ``storage_account_type`` in the cluster definition section.
      * - ``subnetName``
        - Name of the Azure network created by ElastiCluster to attach
          the cluster to.
      * - ``vmName``
        - Node name (generated by ElastiCluster).
      * - ``vmSize``
        - "Flavor" of the VM, from option ``flavor`` in the configuration.
      * - ``bootDiskSize``
        - Size of the boot disk in GiB. See option ``boot_disk_size``
          in the cluster definition section.

``net_deployment_template``
   Path to a JSON file containing the Azure Resource Manager template
   used for creating a cluster's network.  The `default ARM template`_ used by
   ElastiCluster uses the `API profile *2018-03-01-hybrid*`_; however,
   newer features may be available in some zones which are not part of
   the global API profile.  In order to use specific features or more
   modern service API versions, you can instruct ElastiCluster to use
   ...
   Usage is like for ``vm_deployment_template``, which see.
